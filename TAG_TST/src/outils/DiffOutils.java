...
public class DiffOutils {
	  ...
	  protected String[] diff_MyMatch(String text1, String text2) {
		    if (Diff_Timeout <= 0) {
		      // Don't risk returning a non-optimal diff if we have unlimited time.
		      return null;
		    }
		    ...
		    // Suppression du contrôle sur la longueur.
	  ...
	  public List<String> my_diff_Test(String longtext, String recherche, int i) {
		    // On considère l'ensemble de la chaine
		    String seed = longtext; // longtext.substring(i, i + longtext.length() / 4);
		    int j = -1;
		    int position = -1;
		    int curseur = 1;
		    int recherche_start = 0;
		    int recherche_end = recherche.length();
		    List<String> retour= new LinkedList<String>();
		    String temp = new String();
		    String best_common = "";
		    String best_longtext_a = "", best_longtext_b = "";
		    String best_shorttext_a = "", best_shorttext_b = "";
		    // Tant qu'on ne trouve pas le patern dans le texte
		    
		    // Si on trouve tout de suite recherche dans seed
		    if ((j = seed.indexOf(recherche)) != -1) {
		    	position = j;
		    }
		 
		    // On prend un minimum de 4 caractères consécutifs
		    while (recherche_start < recherche_end) {
			    while (curseur < recherche.length() && (j = seed.indexOf(recherche.substring(recherche_start, curseur))) != -1) {
			    	temp = recherche.substring(recherche_start, curseur);
			    	position = j;
			    	System.out.println(temp + ":" + position);
			    	curseur++;
			    	if (temp.isEmpty() || curseur >= recherche_start) {
			    		break;
			    	}
			    }
			    retour.add(temp);
		    ...
		    return retour;
//		    while ((j = recherche.indexOf(seed, j + 1)) != -1) { // Comment cela pourrais être vrai ???
//		      int prefixLength = diff_commonPrefix(longtext.substring(i), recherche.substring(j));
//		      int suffixLength = diff_commonSuffix(longtext.substring(0, i), recherche.substring(0, j));
//		      if (best_common.length() < suffixLength + prefixLength) {
//		        best_common = recherche.substring(j - suffixLength, j) + recherche.substring(j, j + prefixLength);
//		        best_longtext_a = longtext.substring(0, i - suffixLength);
//		        best_longtext_b = longtext.substring(i + prefixLength);
//		        best_shorttext_a = recherche.substring(0, j - suffixLength);
//		        best_shorttext_b = recherche.substring(j + prefixLength);
//		      }
//		    }
//		    if (best_common.length() * 2 >= longtext.length()) {
//		      return new String[]{best_longtext_a, best_longtext_b,
...
//		    		}
	   ...
	   * @return Best match index or -1.
	   */
	  public int match_MyBitap(String text, String pattern, int loc) {
